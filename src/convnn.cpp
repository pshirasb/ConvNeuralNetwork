...
double NeuralNet::checkGradients(cube x,cube y){
    ...
    for(Layer* l = _layers_head; l; l = l->next){
        ...
        do {
            ...
            if(abs(gradApprox - old_gradient) > 1e-6 ) {
                cout << "Incorrect gradient: " << abs(gradApprox - old_gradient) << endl;
            }
...
bool maxPoolLayer::connectBack (Layer *l) {
    ...
    dbg_assert((nr % _sample_size) == 0);
    dbg_assert((nc % _sample_size) == 0);

    _mask     = cube(nr, nc, ns, fill::zeros);
    _a        = cube(nr / _sample_size, 
                     nc / _sample_size,
                     ns,
                     fill::zeros);
    _units = ns;
    ...
    dbg_print("connect back from maxPoolLayer with " << _units << " units." );
    return(0);
